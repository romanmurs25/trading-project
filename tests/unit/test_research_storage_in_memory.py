...
def test_in_memory_storage_saves_lists_and_gets_research_run() -> None:
...
def test_in_memory_storage_saves_and_loads_research_results_equity_and_trades() -> None:
    storage = InMemoryStorage()
    result = make_result()
    points = [
        BacktestEquityPoint(
    ...
    storage.save_backtest_equity_points(points)
    storage.save_backtest_trade_records(records)
    ...
    assert storage.load_backtest_equity_points("bt-1") == points
    assert storage.load_backtest_trade_records("bt-1") == records
